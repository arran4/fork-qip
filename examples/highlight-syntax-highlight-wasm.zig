...
fn codeTagHasLanguageWasm(tag: []const u8) bool {
    ...
    while (i < tag.len) {
        ...
        if (eqlIgnoreCase(name, "class") and classContainsLanguageWasm(value)) return true;
    }
    return false;
}
...
fn parseCodeOpenTag(input: []const u8, start: usize) ?CodeOpenTag {
    ...
    const has_language_wasm = codeTagHasLanguageWasm(input[start .. end + 1]);
    return .{ .end = end, .has_language_wasm = has_language_wasm };
}
...
fn transformHTML(input: []const u8, w: *Writer) void {
    ...
    while (i < input.len) {
        ...
        const should_highlight = open.has_language_wasm and std.mem.indexOfScalar(u8, inner, '<') == null;
        if (!should_highlight) {
...
test "highlights plain text language-wasm code blocks" {
    ...
    const expected = "<pre><code class=\"language-wasm\">(<span class=\"hljs-keyword\">module</span> (<span class=\"hljs-keyword\">func</span> <span class=\"hljs-variable\">$add</span> (<span class=\"hljs-keyword\">param</span> <span class=\"hljs-variable\">$a</span> <span class=\"hljs-type\">i32</span>) (<span class=\"hljs-keyword\">param</span> <span class=\"hljs-variable\">$b</span> <span class=\"hljs-type\">i32</span>) (<span class=\"hljs-keyword\">result</span> <span class=\"hljs-type\">i32</span>) <span class=\"hljs-built_in\">local.get</span> <span class=\"hljs-variable\">$a</span> <span class=\"hljs-built_in\">local.get</span> <span class=\"hljs-variable\">$b</span> <span class=\"hljs-built_in\">i32.add</span>))</code></pre>";
    try std.testing.expectEqualStrings(expected, got);
}

test "skips code blocks that already contain spans" {
    const input = "<code class=\"language-wasm\"><span class=\"hljs-keyword\">module</span></code>";
    const got = runForTest(input);
