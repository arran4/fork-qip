(module
  ...
  (func $fail (param $code i32) (param $in_ptr i32) (param $in_size i32) (param $out_size i32) (result i32)
  ...
  (func $bytes_equal (param $a i32) (param $b i32) (param $n i32) (result i32)
    (local $i i32)
  ...
  (func (export "comply") (result i32)
    ...
    (if (i32.lt_u (call $input_utf8_cap) (i32.const 64))
      (then
        (return (call $fail (i32.const -100) (i32.const 0) (i32.const 0) (i32.const 0)))
      )
    )

    (local.set $in_ptr (call $input_ptr))
    (local.set $expected_ptr (i32.add (local.get $in_ptr) (i32.const 32)))
    (local.set $pass_count (i32.const 0))
    ...
    (if (i32.ne (local.get $out_size) (i32.const 12))
      (then
        (return (call $fail (i32.const -1) (local.get $in_ptr) (i32.const 17) (local.get $out_size)))
      )
    )
    (memory.init $case1_out (local.get $expected_ptr) (i32.const 0) (i32.const 12))
    (local.set $out_ptr (call $output_ptr))
    (if (i32.eqz (call $bytes_equal (local.get $out_ptr) (local.get $expected_ptr) (i32.const 12)))
      (then
        (return (call $fail (i32.const -1) (local.get $in_ptr) (i32.const 17) (local.get $out_size)))
      )
    )
    (local.set $pass_count (i32.add (local.get $pass_count) (i32.const 1)))
    ...
    (if (i32.ne (local.get $out_size) (i32.const 12))
      (then
        (return (call $fail (i32.const -2) (local.get $in_ptr) (i32.const 17) (local.get $out_size)))
      )
    )
    ...
    (if (i32.ne (local.get $out_size) (i32.const 8))
      (then
        (return (call $fail (i32.const -3) (local.get $in_ptr) (i32.const 7) (local.get $out_size)))
      )
    )
    (memory.init $case3_out (local.get $expected_ptr) (i32.const 0) (i32.const 8))
    (local.set $out_ptr (call $output_ptr))
    (if (i32.eqz (call $bytes_equal (local.get $out_ptr) (local.get $expected_ptr) (i32.const 8)))
      (then
        (return (call $fail (i32.const -3) (local.get $in_ptr) (i32.const 7) (local.get $out_size)))
      )
    )
    ...
    (if (i32.ne (local.get $out_size) (i32.const 0))
      (then
        (return (call $fail (i32.const -4) (local.get $in_ptr) (i32.const 0) (local.get $out_size)))
      )
    )
    (local.set $pass_count (i32.add (local.get $pass_count) (i32.const 1)))
    ...
    (if (i32.ne (local.get $out_size) (i32.const 0))
      (then
        (return (call $fail (i32.const -5) (local.get $in_ptr) (i32.const 3) (local.get $out_size)))
      )
    )
    (local.set $pass_count (i32.add (local.get $pass_count) (i32.const 1)))
    ...
    (if (i32.ne (local.get $out_size) (i32.const 0))
      (then
        (return (call $fail (i32.const -6) (local.get $in_ptr) (i32.const 1) (local.get $out_size)))
      )
    )
    (local.set $pass_count (i32.add (local.get $pass_count) (i32.const 1)))
    ...
    (local.set $out_size (call $run (i32.const 17)))
    (if (i32.ne (local.get $out_size) (i32.const 15))
      (then
        (return (call $fail (i32.const -7) (local.get $in_ptr) (i32.const 17) (local.get $out_size)))
      )
    )
    (memory.init $case7_out (local.get $expected_ptr) (i32.const 0) (i32.const 15))
    (local.set $out_ptr (call $output_ptr))
    (if (i32.eqz (call $bytes_equal (local.get $out_ptr) (local.get $expected_ptr) (i32.const 15)))
      (then
        (return (call $fail (i32.const -7) (local.get $in_ptr) (i32.const 17) (local.get $out_size)))
      )
    )
    ...
    (if (i32.ne (local.get $out_size) (i32.const 4))
      (then
        (return (call $fail (i32.const -8) (local.get $in_ptr) (i32.const 5) (local.get $out_size)))
      )
    )
    (memory.init $case8_out (local.get $expected_ptr) (i32.const 0) (i32.const 4))
    (local.set $out_ptr (call $output_ptr))
    (if (i32.eqz (call $bytes_equal (local.get $out_ptr) (local.get $expected_ptr) (i32.const 4)))
      (then
        (return (call $fail (i32.const -8) (local.get $in_ptr) (i32.const 5) (local.get $out_size)))
      )
    )
    ...
    ;; All checks passed; return the number of passing checks.
    (local.get $pass_count)
  )
)
