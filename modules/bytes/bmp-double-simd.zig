...
const DUP_MASK = @Vector(16, i32){ 0, 1, 2, 3, 0, 1, 2, 3, 4, 5, 6, 7, 4, 5, 6, 7 };
...
export fn run(input_size_in: u32) u32 {
    ...
    while (y < abs_height) : (y += 1) {
        ...
        var x: u32 = 0;
        const simd_limit = uwidth & ~@as(u32, 1);
        while (x < simd_limit) : (x += 2) {
            const px_idx: u32 = src_row_ptr + x * 4;
            const v8 = @as(*align(1) const Vec8, @ptrCast(&input_buf[@intCast(px_idx)])).*;
            const v16 = @shuffle(u8, v8, v8, DUP_MASK);

            const d0: u32 = dst_row_ptr0 + x * 8;
            const d1: u32 = dst_row_ptr1 + x * 8;
            const out0 = @as(*align(1) Vec16, @ptrCast(&output_buf[@intCast(d0)]));
            const out1 = @as(*align(1) Vec16, @ptrCast(&output_buf[@intCast(d1)]));
            out0.* = v16;
            out1.* = v16;
        }
        if (x < uwidth) {
